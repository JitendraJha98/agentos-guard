...
- **The latency gate is genuinely failing on this host right now**, and it is NOT a Phase-14 regression: A/B'd against the pre-Phase-14 merge base `8b04ed7`, which fails it at mean 7.371 ms versus 6.756 ms on the Phase-14 branch — the branch is faster than the base. The same host measured 4.15 ms earlier in the session, so the box has degraded. Re-measure on a quiet machine before treating this as a code problem.
