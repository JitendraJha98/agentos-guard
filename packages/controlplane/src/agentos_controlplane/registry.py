...
class Registry:
    ...
    def register(
        ...
        DISC-01: an optional registration `manifest` ({tools,prompts,memories}) declares
        authoritative inventory rows when an InventoryStore is wired. Both are optional, so
        register(agent_id) / register(agent_id, trust_score) stay backward compatible.
        """
