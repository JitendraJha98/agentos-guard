...
Keyed by the catalogue `name`: the row is the CURRENT observation per framework, upserted by each
scan and bracketed by first/last-seen. The immutable history lives on the audit chain, where a
newly-observed framework (or a version change) appends one `framework_discovered` event carrying
short identifiers only — a repeat scan over an unchanged environment appends nothing, so a
scheduled pass cannot bloat the chain.
