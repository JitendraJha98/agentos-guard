...
runs it through the same in-process pipeline via the shared enforcement core
(`governed_call`), and — on allow — executes the wrapped operation; on deny it raises
`GovernanceDenied` WITHOUT executing it (no side effect: no memory write, no MCP egress,
...
async def governed_memory_access(
    ...
    *,
    ...
    return await governed_call(pipeline, action, run)
...
async def governed_mcp_call(
    ...
    *,
    ...
    """Govern an MCP-server call (INT-04). Allow runs `run`; deny raises GovernanceDenied."""
...
async def governed_delegation(
    ...
    *,
    ...
    """Govern an agent-to-agent delegation (INT-05), capturing `parent_action_id` lineage.

    Allow dispatches the sub-agent (`run`); deny raises GovernanceDenied without dispatch.
    """
