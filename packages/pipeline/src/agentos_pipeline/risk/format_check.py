...
INVARIANTS (tested):
  - inline=True, pure CPU, sub-ms. NO model, NO network, NO LLM on the hot path
    (P0-killer Pitfall 1). stdlib only; the walk is ITERATIVE (a hostile
    deeply-nested payload cannot blow the recursion limit).
...
_FORBIDDEN_CONTROL = {chr(c) for c in range(0x20)} - {"\t", "\n", "\r"}
        ...
        while stack:
            ...
            if isinstance(node, dict):
                ...
                if len(node) > _MAX_VALUE_LEN:
                    oversized = True
                elif not control_chars and any(
                    ch in _FORBIDDEN_CONTROL for ch in node
                ):
                    control_chars = True
